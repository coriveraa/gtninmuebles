...
Class ModeloLandingPage{
      
 

    static public function mdlMostrarLandingPage ($tabla){
    ...
    static public function mdlMostrarMenuPrincipal ($tabla){
                         
        $stmt = Conexion::conectar ()-> prepare("SELECT * FROM $tabla WHERE pagina='inicio' and activo = 1");
...
/*=============================================
    ...
    static public function mdlMostrarCategorias ($tabla, $item, $valor){
		...
		$stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla where pagina='blog'");
        ...
        $stmt = null;
            ...
            static public function mdlMostrarMenuBlog ($tabla){
	...
	static public function mdlMostrarConInnerJoin($tabla1, $tabla2, $desde, $cantidad, $item, $valor){
		...
		if($item == null && $valor == null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT $tabla1.*, $tabla2.*, DATE_FORMAT(fecha_articulo, '%d.%m.%Y') AS fecha_articulo FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.id_categorias = $tabla2.id_cat ORDER BY $tabla2.id_articulo DESC LIMIT $desde, $cantidad");
			...
			$stmt = Conexion::conectar()->prepare("SELECT $tabla1.*, $tabla2.*, DATE_FORMAT(fecha_articulo, '%d.%m.%Y') AS fecha_articulo FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.id_categorias = $tabla2.id_cat WHERE $item = :$item ORDER BY $tabla2.id_articulo DESC LIMIT $desde, $cantidad");
	...
	static public function mdlBuscador($tabla1, $tabla2, $desde, $cantidad, $busqueda){
		...
		$stmt = Conexion::conectar()->prepare("SELECT $tabla1.*, $tabla2.*, DATE_FORMAT(fecha_articulo, '%d.%m.%Y') AS fecha_articulo FROM $tabla1 INNER JOIN $tabla2 ON $tabla1.id_categorias = $tabla2.id_cat WHERE titulo_articulo like '%$busqueda%' OR descripcion_articulo like '%$busqueda%' OR contenido_articulo like '%$busqueda%' OR ruta_articulo like '%$busqueda%' ORDER BY $tabla2.id_articulo DESC LIMIT $desde, $cantidad");
	...
	static public function ctrMostrarPropiedades($tabla){
	...
	static public function ctrMostrarCartas($tabla){
